// <auto-generated />
using System;
using Data.Context;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

namespace Data.Migrations
{
    [DbContext(typeof(ControleCargaContext))]
    [Migration("20201027143356_AlterNameTable_temMovim")]
    partial class AlterNameTable_temMovim
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "3.1.9")
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("Business.Models.Cliente", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<decimal>("Codigo")
                        .HasColumnType("numeric(5)");

                    b.Property<string>("Descricao")
                        .IsRequired()
                        .HasColumnType("varchar(100)");

                    b.HasKey("Id");

                    b.ToTable("Cliente");
                });

            modelBuilder.Entity("Business.Models.ClienteProduto", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long>("ClienteId")
                        .HasColumnType("bigint");

                    b.Property<string>("CodigoBarra")
                        .IsRequired()
                        .HasColumnType("varchar(100)");

                    b.Property<long>("ProdutoId")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.HasIndex("ClienteId");

                    b.HasIndex("ProdutoId");

                    b.ToTable("ClienteProduto");
                });

            modelBuilder.Entity("Business.Models.Coletor", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Imei")
                        .IsRequired()
                        .HasColumnType("varchar(25)");

                    b.Property<DateTime?>("LastFichaCC")
                        .HasColumnName("Last_Ficha_CC")
                        .HasColumnType("datetime");

                    b.Property<DateTime?>("LastSincCC")
                        .HasColumnName("Last_Sinc_CC")
                        .HasColumnType("datetime");

                    b.Property<decimal>("Numero")
                        .HasColumnType("numeric(3)");

                    b.Property<string>("Observacao")
                        .HasColumnType("varchar(250)");

                    b.Property<decimal>("Status")
                        .HasColumnType("numeric(1)");

                    b.Property<decimal>("UtilizaCC")
                        .HasColumnName("Utiliza_CC")
                        .HasColumnType("numeric(1)");

                    b.HasKey("Id");

                    b.ToTable("Coletor");
                });

            modelBuilder.Entity("Business.Models.Frota", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Descricao")
                        .HasColumnType("varchar(100)");

                    b.Property<string>("Placa")
                        .IsRequired()
                        .HasColumnType("varchar(10)");

                    b.HasKey("Id");

                    b.ToTable("Frota");
                });

            modelBuilder.Entity("Business.Models.ItemMovimento", b =>
                {
                    b.Property<long>("MovimentoId")
                        .HasColumnType("bigint");

                    b.Property<long>("ClienteProdutoId")
                        .HasColumnName("cliprdID")
                        .HasColumnType("bigint");

                    b.Property<decimal>("Qtd")
                        .HasColumnType("numeric(5)");

                    b.HasKey("MovimentoId", "ClienteProdutoId");

                    b.HasIndex("ClienteProdutoId");

                    b.ToTable("Item_Movimento");
                });

            modelBuilder.Entity("Business.Models.Movimento", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("ColetorApp")
                        .HasColumnName("Coletor_App")
                        .HasColumnType("varchar(15)");

                    b.Property<string>("ColetorChave")
                        .HasColumnName("Coletor_Chave")
                        .HasColumnType("varchar(20)");

                    b.Property<decimal>("ColetorDoc")
                        .HasColumnName("Coletor_Doc")
                        .HasColumnType("numeric");

                    b.Property<long>("ColetorId")
                        .HasColumnType("bigint");

                    b.Property<DateTime?>("DataHoraFinal")
                        .HasColumnName("Data_Hora_Final")
                        .HasColumnType("datetime");

                    b.Property<DateTime?>("DataHoraGravacao")
                        .HasColumnName("Data_Hora_Gravacao")
                        .HasColumnType("datetime");

                    b.Property<DateTime?>("DataHoraInicial")
                        .HasColumnName("Data_Hora_Inicial")
                        .HasColumnType("datetime");

                    b.Property<long>("FrotaId")
                        .HasColumnType("bigint");

                    b.Property<string>("Obs")
                        .HasColumnType("varchar(500)");

                    b.Property<long>("UsuarioId")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.HasIndex("ColetorId");

                    b.HasIndex("FrotaId");

                    b.HasIndex("UsuarioId");

                    b.ToTable("Movimento");
                });

            modelBuilder.Entity("Business.Models.Produto", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<decimal>("Codigo")
                        .HasColumnType("numeric(5)");

                    b.Property<string>("Descricao")
                        .IsRequired()
                        .HasColumnType("varchar(100)");

                    b.HasKey("Id");

                    b.ToTable("Produto");
                });

            modelBuilder.Entity("Business.Models.Usuario", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<decimal>("Codigo")
                        .HasColumnType("numeric(5)");

                    b.Property<string>("Login")
                        .IsRequired()
                        .HasColumnType("varchar(10)");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasColumnType("varchar(50)");

                    b.Property<string>("Senha")
                        .IsRequired()
                        .HasColumnType("varchar(10)");

                    b.HasKey("Id");

                    b.ToTable("Usuario");
                });

            modelBuilder.Entity("Business.Models.ClienteProduto", b =>
                {
                    b.HasOne("Business.Models.Cliente", "Cliente")
                        .WithMany("ClienteProdutos")
                        .HasForeignKey("ClienteId")
                        .IsRequired();

                    b.HasOne("Business.Models.Produto", "Produto")
                        .WithMany("ClientesProduto")
                        .HasForeignKey("ProdutoId")
                        .IsRequired();
                });

            modelBuilder.Entity("Business.Models.ItemMovimento", b =>
                {
                    b.HasOne("Business.Models.ClienteProduto", "ClienteProduto")
                        .WithMany()
                        .HasForeignKey("ClienteProdutoId")
                        .IsRequired();

                    b.HasOne("Business.Models.Movimento", "Movimento")
                        .WithMany("ItemMovimento")
                        .HasForeignKey("MovimentoId")
                        .IsRequired();
                });

            modelBuilder.Entity("Business.Models.Movimento", b =>
                {
                    b.HasOne("Business.Models.Coletor", "Coletor")
                        .WithMany()
                        .HasForeignKey("ColetorId")
                        .IsRequired();

                    b.HasOne("Business.Models.Frota", "Frota")
                        .WithMany()
                        .HasForeignKey("FrotaId")
                        .IsRequired();

                    b.HasOne("Business.Models.Usuario", "Usuario")
                        .WithMany()
                        .HasForeignKey("UsuarioId")
                        .IsRequired();
                });
#pragma warning restore 612, 618
        }
    }
}
